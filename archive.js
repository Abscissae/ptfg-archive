...
function search(H)
{
  ...
  const A = document.querySelectorAll(document.getElementById("sortedstories").hasChildNodes()?"#sortedstories":"main section");
  let d = 0;
  for (i = 0; i < A.length; i++)
  {
    const S = A[i].getElementsByTagName("article");
    let c = 0;
...
function sortBy(v)
{
	const N = document.getElementById("sortedstories");
	const M = document.getElementById("storiesbyauthor");
	N.replaceChildren();
	if (v==="Author")
	{
		M.style.display="";
	...
	const s = document.querySelectorAll("#storiesbyauthor article");
